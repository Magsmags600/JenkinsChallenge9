...
//   // Remove city method to remove a city by ID from db.json
//   async removeCity(id: string): Promise<void> {
//     let cities = await this.read();
//     cities = cities.filter((city) => city.id !== id);
//     await this.write(cities);
...
class HistoryService {
  ...
  private async read(): Promise<City[]> {
    try {
      ...
      // Ensure cities key exists and is an array
      return parsedData.cities || [];
    } catch (error) {
      console.error('Error reading search history:', error);
      return [];
  ...
  private async write(cities: City[]): Promise<void> {
    try {
      // Ensure cities are saved under the 'cities' key in db.json
      const dbContent = { cities };
      await fs.writeFile(dbFilePath, JSON.stringify(dbContent, null, 2), 'utf-8');
    } catch (error) {
      console.error('Error writing to search history:', error);
      throw error;
